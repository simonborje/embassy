    ...
    fn rx(&mut self, packet: &mut [u8]) {
        ...
        match channel {
            ...
            CHANNEL_TYPE_EVENT => {
                ...
                let update_link_status = match (event_type, status, event_packet.msg.flags, event_packet.msg.reason) {
                    // Ignore unsolicited and successful AUTH messages
                    (Event::AUTH, EStatus::UNSOLICITED | EStatus::SUCCESS, ..) => false,
                    ...
                    // Successfully joined the network - this is enough for open networks while secure networks also require a successful key exchange
                    ...
                    (Event::PSK_SUP, EStatus::UNSOLICITED, 0, 0) => {
                        self.keys_exchanged_status = true;
                        true
                    }
                    // Other PSK_SUP events indicate authentication errors
                    (Event::PSK_SUP, ..) => {
                        self.keys_exchanged_status = false;
                        true
                    }
